...
void main ()
{
	...
	int f=0;
	for (int i=0; i<n; i++)
		if (dynamic_cast<Point*>(arr[i]))
		{
			arr1c[f]=arr[i];
			f++;
		}
	for (int i=0; i<f-1; i++)
		if (arr1c[i]->rasst()>arr[i+1]->rasst())
		{
				temp=arr1c[f];
				arr1c[f]=arr1c[f+1];
				arr1c[f+1]=temp;
	...
	for (int i=0; i<n; i++)
		if (dynamic_cast<Circle*>(arr[i]))
			{
				Circle* ptr=reinterpret_cast<Circle*> (arr[i]);
				arrc[u]=ptr;
				u++;
			}
		cout << u << endl;
		for (int j=0; j<u; j++)
			...
			{
				cout << "Расстояние от центра " << j << "-ая окружность. С радиусом " << arrc[j]->r << endl;
				outfile << "Расстояние от центра " << j << "-ой окружность. С радиусом " << arrc[j]->r << endl;
			}
