...
void main ()
{
	...
	for (int i=0; i<f; i++)
		cout << "Расстояние " << i+1 << "-ой точки от центра:  " << arr1c[i]->rasst() << endl;
	cout << "Точек в массиве: " << f << endl;
	outfile << "Точек в массиве: " << f << endl;
	for (int i=0; i<f; i++)
		outfile << "Расстояние " << i+1 << "-ой точки от центра:  " <<  arr1c[i]->rasst() << endl;
	...
	for (int i=0; i<n; i++)
		...
		outfile << "Окружностей в массиве: " << u << endl; 
		for (int j=0; j<u; j++)
