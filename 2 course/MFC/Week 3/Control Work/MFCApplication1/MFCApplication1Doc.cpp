...
void CMFCApplication1Doc::Serialize(CArchive& ar)
{
	if (ar.IsStoring())
	{
		int nCount = students.size();

		ar << nCount;

		for (vector<pair<CString, pair<CString, int>>>::iterator it = students.begin(); it != students.end(); ++it)
		{
	...
	else
	{
		...
		for (; nCount > 0; nCount--)
		{
			...
			ar >> b;
			ar >> y;
			students.push_back(make_pair(a, make_pair(b, y)));
		}
